...
#define ICON_Clothing "m"
    ...
    ImVec2 optionSizeBox = ImVec2(menuSize.x, 320);
    ...
    float rightSideTextOffet = 62.5f;
    ...
    void DrawMainMenu() {
        ...
        ImDrawCornerFlags cornerFlags = useAllCorners ? ImDrawCornerFlags_All : ImDrawCornerFlags_TopLeft;
        ...
        if (horizontalCurrentWidth > 0.0f) {
            ...
            draw::RectFilled(defaultBackgroundColor, { menuPos.x + currentSize.x, menuPos.y }, { horizontalMenuSize.x - mainMenuSize.x, verticalMenuSize.y }, ImDrawCornerFlags_TopRight, 7.5f);
        }
    }
    ...
    bool Sub(std::string text) {
        ...
        if (subMenuOptionCount <= maxIconsPerColumn) {
            ...
            int indexInRow = subMenuOptionCount - maxIconsPerColumn;
            yPos = menuPos.y + maxSize.y + textOffsetX;
            xPos = menuPos.x + indexInRow * (iconWidth + (textOffsetX + 10.f));
        }
    ...
    void End() {
        ...
        draw::Text("v!.0.0", fadedTextColor, ImVec2((textPos.x - rightSideTextOffet) - 2.5f, textPos.y + 70.f), m_font_small, true);
    ...
    void DrawDebugMenu() {
        if (ImGui::Begin("Debug Menu")) {
            ...
            ImGui::SliderInt("Max Options", &maxOption, 1, 20);
